...
nodes-own [
  ...
  prod_block? ;; boolean showing if a node produses a block
  block_con_id ;; the id of the block to connect the new block to
]

globals [
  blockchain ;; list that holds all the ids of all blocks
  forks  ;; list that holds the ids of all blocks that produce a fork in the chain
  ...
  y ;; holds the y coordinate of the main blockchain, used to position correctly
]
...
to setup
  ...
  setup-nodes
  set max_depth 0 ;;
  produce_block 0 0 0;; create the first block, Υποθεση για σωστη λειτουργια του κωδικα
  ask blocks [
    set depth 1
    set y 5
  ]
...
to go
  ...
  ;; if a new block was produced, all nodes restart the production of a new block
  if new_b? [
    set new_b? false
    set max_depth max_depth + 1
    set new_block_tick ticks  ;; mark the tick that a new block started being produced
    ask nodes [set nonce 1 + random-poisson mean-ticks] ;; get a new nonce for the new block
  ]

  ;; all nodes check each tick if they want to propose a block
  ask nodes [
    ;; use the current tick and the new_block_tick to check if a node produces a block
  ...
  repeat count nodes with [ prod_block? = true ] [
    ...
    ask one-of nodes with [ prod_block? = true ] [
      ...
      set fork_block block_con_id ;; keep in the local var the id of the suitable block
    ]
...
;; reports the id of the most suitable block based on the mechanism we choose
to-report suitable_block
  ...
  if mode = "Longest Chain" [
   ...
   if mode = "Heaviest Chain" [
    ...
    ;; suitable block is the one with the deepest block with the biggest weight
    set s_block [id] of one-of blocks with [depth = max_depth and weight = max weights_list]
  ]
...
;  last [ycor] of blocks with [id = p_id] ;; var that holds the ycor of the parent block
...
;  ]
  ...
  ;; create link with the parent block for better visualization of the chain
