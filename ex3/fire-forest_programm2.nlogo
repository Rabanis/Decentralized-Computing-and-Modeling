...
patches-own [
  slow?           ;; bollean to set a tree as slow burning
]
...
to setup
  ...
  ask patches with [(random-float 100) < density] [
    ...
    ;; if a tree is slow burning, it's yellow
    if random-float 100 < slow-burning-trees [
      set slow? true
      set pcolor yellow
    ]
  ]
  ...
  ask patches with [pxcor = min-pxcor]
    [ ignite ]
  ;; set tree counts
  set initial-trees count patches with [pcolor = green or pcolor = yellow]
  set burned-trees 0

  ;; create a road in the middle spanning half the forest
  if road? [
...
to go
  ...
  ask fires [
;;;;;; with moore neighborhood, all 8 of the neighbors are ignited
    ifelse moore? [

      ;; with probability q the fire spreads from the current tree
      if random 100 <= q [
        ask neighbors with [pcolor = green or pcolor = yellow]
        [ ignite ]

        ;; wind transfers the fire for "wind-intensity" patches to the right
        if wind? and random 100 < active-wind [

          ;; if the wind intensity is more than 1, then set ablaze the trees to the right of
          ;; my right neighbor, so all of its neighbors.
          if wind-intensity >= 2 [
            ;; safety measure for the edges of the world
            if pxcor < max-pxcor - 2 [
              ask patch (pxcor + 1) pycor [
                ask neighbors with [pcolor = green or pcolor = yellow] [ force-ignite ]
              ]
            ]
          ]

          ;; if the wind intensity is 3, then set ablaze the trees 2 trees
          ;; away form my neighbors
          if wind-intensity = 3 [
            if pxcor < max-pxcor - 3 [
...
;;;;;; with von-neuman neighborhood, only 4 neighbors are considered
    [
      ;; with probability q the fire spreads from the current tree
      if random 100 <= q [
        ask neighbors4 with [pcolor = green or pcolor = yellow]
        [ ignite ]

        ;; wind transfers the fire for "wind-intensity" patches to the right
        if wind? and random 100 < active-wind [

          ;; if the wind intensity is more than 1, then set ablaze the tree one tree
          ;; away form my neighbors
          if wind-intensity >= 2 [
            if pxcor < max-pxcor - 2 [
              ask patch (pxcor + 2) pycor [
                if pcolor = green or pcolor = yellow [ force-ignite ]
              ]
            ]
          ]
    ...
    ;; simulate the slow burn of the trees
    ifelse [slow?] of patch-here = false [
      set color red
      set breed embers
    ]
    [
      ;; if the patch is a slow burning tree, make the flame white
      ;; and set slow? false to make it an ember next round
      set slow? false
      set color white
    ]
...
to ignite  ;; patch procedure
  ...
  ;; get for each patch that calls the procedure the current neighbor burning trees number
  let ignited-neig count neighbors with [any? fires-here]
  ;; if the ticks are 0, then setup the starting line of fire
  if ticks = 0 [
    force-ignite
    set burned-trees 0
  ]
  ;; if the number of ignited neighbors is less than the tenacity, don't ignite
  if ignited-neig >= tenacity [
    force-ignite
  ]
end


to force-ignite ;; patch procedure
  sprout-fires 1
    [ set color red ]
    set pcolor black
...
<experiments>
  <experiment name="experiment" repetitions="10" runMetricsEveryStep="false">
    ...
    <enumeratedValueSet variable="slow-burning-trees">
      <value value="0"/>
    </enumeratedValueSet>
    ...
    <enumeratedValueSet variable="wind-intensity">
      <value value="1"/>
      <value value="2"/>
      <value value="3"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="moore?">
      <value value="true"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="wind?">
      <value value="true"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="active-wind">
      <value value="100"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="tenacity">
      <value value="1"/>
    </enumeratedValueSet>
  ...
  <experiment name="q" repetitions="5" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>(burned-trees / initial-trees) * 100</metric>
    <enumeratedValueSet variable="slow-burning-trees">
    ...
    <steppedValueSet variable="q" first="80" step="1" last="100"/>
    <enumeratedValueSet variable="road?">
      <value value="false"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="slow" repetitions="5" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>(burned-trees / initial-trees) * 100</metric>
    <steppedValueSet variable="slow-burning-trees" first="30" step="15" last="75"/>
    <enumeratedValueSet variable="density">
      <value value="65"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="wind-intensity">
    ...
    <steppedValueSet variable="q" first="70" step="5" last="90"/>
    <enumeratedValueSet variable="road?">
      <value value="false"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="test" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>(burned-trees / initial-trees) * 100</metric>
    <enumeratedValueSet variable="slow-burning-trees">
    ...
    <steppedValueSet variable="density" first="25" step="5" last="60"/>
    <enumeratedValueSet variable="wind-intensity">
      <value value="1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="moore?">
